...
// Example cell value: "Rose, Light Blue, Mint Green, Multi"
const COLOUR_MAP = {
  // Pinks
  "rose":         "#C9697E",
  "light pink":   "#F9C6D4",
  ...
  "beige":        "#F0DEC8",
  "brown":        "#A07858",
  // Special — rendered as conic rainbow gradient via CSS class
  "multi":        "__MULTI__",
};
...
/**
 * Resolve a colour string from the sheet to a { hex, name, isMulti } object.
 * Accepts:
 *   - Named colours from COLOUR_MAP  ("Light Blue", "multi", etc.)
 *   - Raw hex / CSS colours          ("#B3D9F2", "lightblue", "rgb(…)")
 */
function resolveColour(raw) {
  ...
  // Fall back to whatever CSS value was given (hex, rgb, named CSS colour…)
...
function buildColorSwatches(colorStr, productIndex) {
  ...
  if (selectedColors[productIndex] === undefined) {
    selectedColors[productIndex] = colours[0].isMulti ? "Multi" : colours[0].hex;
  }
...
function renderCart() {
  ...
  list.innerHTML = cart.map((item, i) => {
    // Work out how to display the colour in the cart
    ...
    const colorDotStyle = isMultiColor
      ...
      : `background:${escHtml(item.chosenColor || "#ccc")};`;
